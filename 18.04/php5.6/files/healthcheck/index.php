 ...
 *
 * This is the PHP 5.6/7.0 variant: no scalar type declarations, no short
 * list destructuring, no null coalescing, and Exception rather than
 * Throwable. Keep it in sync with the 7.1+ copy in the other images.
 */
...
$engine = isset($aliases[$driver]) ? $aliases[$driver] : $driver;

try {
    list($dsn, $pdoDriver) = healthcheck_dsn($engine, $host, $port, $name);
 ...
 *
 * @return array [dsn, required PDO driver name]
 */
function healthcheck_dsn($engine, $host, $port, $name)
{
    $port = (int) $port;
