#!/bin/bash

architecture=`arch`
if [ ${architecture} = 'arm64' ]; then
  echo "ARM64 Processor - Using buildx."
  if [[ ${1} = 'production' ]] || [[ ${1} = 'prod' ]]; then
    docker buildx build --platform linux/amd64 -t proboci/ubuntu:20.04-php8.0-mysql5.7 --push .
  else
    docker buildx build --platform linux/amd64 -t proboci/ubuntu:20.04-php8.0-mysql5.7 .
  fi
else
  echo "x86_64 Processor - using build."
  docker build . -t proboci/ubuntu:20.04-php8.0-mysql5.7

  if [[ ${1} = 'production' ]] || [[ ${1} = 'prod' ]]; then
    docker push proboci/ubuntu:20.04-php8.0-mysql5.7
  fi
fi
